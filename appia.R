...
server <- function(input, output) {  
  ...
  ##Data Preprocessing 
  library(readr)
  dataset<- read_csv("C:/Users/BEN HAMMED Wassim/Downloads/Telco-Customer-Churn.csv")
  # Setting "No" values 
  library(plyr)
  ...
  output$plot1 <- renderPlot({
    ...
    plot1 <- ggplot(rv$dataset, aes(x = Churn, fill = Churn)) +
      geom_bar(position = "stack", fill = c("#f0ad4e", "#5cb85c"), color = c("#ba7412", "#376e37")) +
      coord_flip() +
  ...
  output$plot3 <- renderPlot({
    ...
    p1 <- ggplot(rv$dataset, aes(x=Contract, fill = factor(Contract)))+
      geom_bar(position = "stack",  fill = c("#5b94c5","#337ab7","#286192"), color = "#1e496d") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
    ...
    p2 <- ggplot(rv$stayed, aes(x=Contract, fill = factor(Contract)))+
      geom_bar(position = "stack", fill = c("#7cc67c", "#5cb85c", "#499349"), color = ("#376e37")) +
      coord_flip() +
    ...
    p3 <- ggplot(rv$churn, aes(x=Contract, fill = factor(Contract)))+
      geom_bar(position = "stack", fill = c("#f3bd71","#f0ad4e", "#d89b46"), color = "#ba7412") +
      coord_flip() +
  ...
  output$plot4 <- renderPlot({
    ...
    p1 <- ggplot(rv$dataset, aes(x=PaymentMethod, fill = factor(PaymentMethod)))+
      geom_bar(position = position_dodge(width = 0.9),  fill = c("#99bcdb","#5b94c5","#337ab7","#286192"), color = "#1e496d") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "All Customers", fill = '')+
    ...
    p2 <- ggplot(rv$stayed, aes(x=PaymentMethod, fill = factor(PaymentMethod)))+
      geom_bar(position = "stack", fill = c("#addbad","#7cc67c", "#5cb85c", "#499349"), color = ("#376e37")) +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$stayed)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9),hjust = -0.1) +
      labs(title = "Customers Stayed", fill = '')+
    ...
    p3 <- ggplot(rv$churn, aes(x=PaymentMethod, fill = factor(PaymentMethod)))+
      geom_bar(position = "stack", fill = c("#f7d6a6","#f3bd71","#f0ad4e", "#d89b46"), color = "#ba7412") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$churn)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "Customers Left", fill = '')+
  ...
  output$plot6 <- renderPlot({
    ...
    p1 <- ggplot(rv$dataset, aes(x=PaperlessBilling, fill = factor(PaperlessBilling)))+
      geom_bar(position = "stack",  fill = c("#337ab7","#286192"), color = "#1e496d") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "All Customers", fill = '')+
    ...
    p2 <- ggplot(rv$stayed, aes(x=PaperlessBilling, fill = factor(PaperlessBilling)))+
      geom_bar(position = "stack", fill = c("#5cb85c", "#499349"), color = ("#376e37")) +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$stayed)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9),hjust = -0.1) +
      labs(title = "Customers Stayed", fill = '')+
    ...
    p3 <- ggplot(rv$churn, aes(x=PaperlessBilling, fill = factor(PaperlessBilling)))+
      geom_bar(position = "stack", fill = c("#f0ad4e", "#d89b46"), color = "#ba7412") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$churn)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "Customers Left", fill = '')+
  ...
  output$plot10 <- renderPlot({
    ...
    p1 <- ggplot(rv$dataset, aes(x=InternetService, fill = factor(InternetService)))+
      geom_bar(position = "stack",  fill = c("#99bcdb","#5b94c5","#286192"), color = "#1e496d") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "All Customers", fill = '')+
    ...
    p2 <- ggplot(rv$stayed, aes(x=InternetService, fill = factor(InternetService)))+
      geom_bar(position = position_dodge(width = 0.9),fill = c("#addbad","#7cc67c", "#499349"), color = ("#376e37")) +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$stayed)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9),hjust = -0.1) +
      labs(title = "Customers Stayed", fill = '')+
    ...
    p3 <- ggplot(rv$churn, aes(x=InternetService, fill = factor(InternetService)))+
      geom_bar(position = position_dodge(width = 0.9), fill = c("#f7d6a6","#f3bd71","#d89b46"), color = "#ba7412") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$churn)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "Customers Left", fill = '')+
  ...
  output$plot11 <- renderPlot({
    p1 <- ggplot(rv$dataset, aes(x=PhoneServ, fill = factor(PhoneServ)))+
      geom_bar(position = position_dodge(width = 0.9),  fill = c("#99bcdb","#5b94c5","#286192"), color = "#1e496d") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$dataset)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "All Customers", fill = '')+
  ...
  output$plot12 <- renderPlot({
    ...
    p2 <- ggplot(rv$stayed, aes(x=Streaming, fill = factor(Streaming)))+
      geom_bar(position = position_dodge(width = 0.9),fill = c("#addbad","#7cc67c", "#5cb85c", "#499349"), color = ("#376e37")) +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$stayed)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9),hjust = -0.1) +
      labs(title = "Customers Stayed", fill = '')+
    ...
    p3 <- ggplot(rv$churn, aes(x=Streaming, fill = factor(Streaming)))+
      geom_bar(position = position_dodge(width = 0.9),fill = c("#f7d6a6","#f3bd71","#f0ad4e", "#d89b46"), color = "#ba7412") +
      coord_flip() +
      geom_text(aes(label = paste(round(..count../nrow(rv$churn)*100,2), "%")),
                stat = "count", position = position_dodge(width = 0.9), hjust = -0.1) +
      labs(title = "Customers Left", fill = '')+
